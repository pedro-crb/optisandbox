...
from optisandbox.numpy.array import length, concatenate
...
def integrate_discrete_intervals(
    ...
    Args:
        ...
        Should be the same length as f and should be monotonically increasing (i.e. x[i] < x[i+1], with no duplicated
        points).

        multiply_by_dx: Whether to multiply the integral by the width of the segment. Defaults to True. - If True,
        summing the integrals will yield the integral of the function over the entire domain (x[0] to x[-1]) - If
        False, you can think of the output as the "average function value" over each interval.

        method: The integration method to use. Options are: - "forward_euler" - "backward_euler" - "trapezoidal" (
        default) - "forward_simpson" - "backward_simpson" - "cubic" Note that some methods, like "cubic", approximate
        each segment interval by looking beyond just the integral itself (i.e., f(a) and f(b)), and so are not
        possible near the endpoints of the array.

        method_endpoints: The integration method to use at the endpoints, for those higher-order methods that require
        handling. Options are: - "lower_order" (default) - "ignore" (i.e. return the integral of the interior points
        only - note that this may result in a different number of integrals than segments!) - "periodic"
    ...
    if method in ["forward_euler", "forward", "euler_forward", "left", "left_riemann"]:
        ...
        if method == "midpoint":
            raise PendingDeprecationWarning(
                "The 'midpoint' method will be deprecated at a future point, since 'trapezoidal' is the more accurate "
                "term here.")
...
def integrate_discrete_squared_curvature(
    ...
    if method in ["cubic", "cubic_spline"]:
        ...
        middle_intervals = 4 * (df ** 2 * (
                s0 * s27 * s9 + s0 * s29 * s6 + s0 * s4 + s1 * s14 + s1 * s15 * s29 + s1 * s16 * s27 - s12 * s16 * s29
                - 2 * s16 * s9 - s17 * s29 * s9 + s23 + s28) + df * dfm * (
                                        2 * h * s17 * s9 - hm * s1 * s13 - hm * s24 * s4 - hm * s25 * s6 - hm * s26 * s9
                                        + 3 * s0 * s3 * s9 + s1 * s17 * s7 - 6 * s18 + s21 - s28) + df * dfp * (
                                        2 * h * s12 * s16 - hp * s0 * s13 - hp * s14 * s24 - hp * s15 * s25 -
                                        hp * s16 * s26 + s0 * s12 * s7 + 3 * s1 * s16 * s3 - 6 * s19 + s22 - s28)
                                + dfm ** 2 * (
        ...
        first_interval = 4 * (
                -2 * h_f * hm_f * s3 * s4 - h_f * hp_f * s5 * s7 + hm_f * hp_f * s10 * s8 - hm_f * s0 * s16 * s34 * (
                    4 * df_f + s29) - 9 * hp_f * s0 * s21 * s28 + s0 * s13 * s3 + s0 * s14 * s5 + s0 * s15 * s22 * (
                            ...
                            dfm_f + dfp_f - s32) + s11 * s12 * s3 + s11 * s18 * s20 + s12 * s14 * s8
                + 6 * s12 * s16 * s30 * (
                            df_f + s29) + s12 * s18 * s39 * (s13 + s38 - 9 * s4) + s12 * s22 * s23 * (
                            s24 - 42 * s4 + s41 + s42 + 3 * s9) + 13 * s13 * s18 * s25
                + 12 * s13 * s22 * s26 + s14 * s20 * s25 + s15 * s16 * s17 + s15 * s18 * s30 * (
                            -7 * dfm_f + s31) - s15 * s25 * s36 * (
                            -8 * dfm_f + s31) + s15 * s26 * s27 + s16 * s23 * s24 + s17 * s21 * s22
                - 4 * s18 * s23 * s33 * (
        ...
        s42 = -s5
        last_interval = 4 * (
                -h_l * hm_l * s3 * s5 - 2 * h_l * hp_l * s6 * s7 + hm_l * hp_l * s10 * s8 - 9
                * hm_l * s0 * s16 * s28 - hp_l * s0 * s21 * s34 * (
                        ...
                        df_l + s29) + s13 * s20 * s25 + 12 * s14 * s15 * s26 +
                13 * s14 * s18 * s25 + s15 * s16 * s17 - s15 * s19 * s30 * (
        ...
        from optisandbox.numpy.calculus import gradient
        dfdx = gradient(
            f,
            x,
            edge_order=2
